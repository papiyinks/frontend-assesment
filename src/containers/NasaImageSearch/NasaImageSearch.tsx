...
const NasaImageSearch = () => {
  ...
  const inputQueryChangeHandler = (event: React.ChangeEvent<HTMLInputElement>): void => {
  ...
  const submitHandler = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const filteredData = data.collection.items.filter((item: { links: []; }) => item.links !== undefined)
  ...
  if (queryResults) {
    ...
    <div className="grid lg:grid-cols-5 md:grid-cols-2 xl:gap-12 gap-8 px-4">
      {queryResults.map((result: { href: string; links: { href: string; }[]; }) => (
        <Card 
          key={result?.href} 
  ...
  if (queryResults?.length === 0) {
    displayedResult = <h1>Sorry, but nothing matched your criteria. Please try again with some different keywords</h1>
  }
    ...
    <div>
      <div className="pt-16 pl-10">
        <form onSubmit={submitHandler} className="flex">
          ...
          <button className="relative py-2 px-8 rounded-md bg-blue-600 hover:bg-blue-500 focus:outline-none" type="submit">
